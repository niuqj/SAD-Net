...
from model.SAD_Net import SADNet
...
def train(net, loader_train, loader_test, optim, criterion):
    ...
    for step in range(start_step + 1, steps + 1):
        ...
        if (step % opt.iters_per_epoch == 0 and step <= opt.finer_eval_step) or (step > opt.finer_eval_step and (step - opt.finer_eval_step) % (5 * len(loader_train)) == 0):
            ...
            # Save the last model at the last step
            if step == steps:
                print(f"\nSaving the final model at epoch {epoch}")
                saved_last_model_path = os.path.join(opt.saved_model_dir, 'last.pk')
                torch.save({
                    'epoch': epoch,
                    'step': step,
                    'max_psnr': max_psnr,
